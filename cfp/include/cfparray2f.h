...
typedef struct {
  ...
  float (*get)(const cfp_array2f* self, uint i);
  float (*get_ij)(const cfp_array2f* self, uint i, uint j);
  void (*set)(cfp_array2f* self, uint i, float val);
  void (*set_ij)(cfp_array2f* self, uint i, uint j, float val);
} Cfp_array2f_api;
